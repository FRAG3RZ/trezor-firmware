...
def select_waveform_groups_by_timestamp(dataset_directory=default_dataset_dir):
    ...
    for time_id in sorted_time_ids:
        ...
        label = f"{time_id} ({num_batteries} batteries, mode: {mode_str}, {count} test{'s' if count != 1 else ''})"
    ...
    phase = inquirer.select(
        ...
        choices=["All", "charging", "charged_relaxing", "discharging", "discharging_relaxing"]
    ).execute()
    ...
    for group in selected_groups:
        for entry in group["files"]:
            if phase == "All" or entry["phase"] == phase:
                battery_file_groups[group["time_id"]][entry["battery_id"]].append(entry["file"])
    ...
    for time_id, battery_dict in battery_file_groups.items():
        ...
        for battery_id, files in battery_dict.items():
            sorted_files = sorted(
                files,
                key=lambda f: PHASE_ORDER.get(f.stem.split(".")[3], 999)
            )
...
def plot_temperature_profile(waveform_name, profile_data):
    ...
    ax[0].set_ylim(bottom=v_min - 0.1 * (v_max - v_min), top=v_max + 0.1 * (v_max - v_min))
...
def plot_group_by_timestamp(time_id, files_dict, ext_temp_file):
    ...
    fig.canvas.manager.set_window_title(
        f"Group: {time_id} - {len(files_dict)} batteries - Mode: {mode}"
    )
    ...
    for i, battery_id in enumerate(battery_ids):
        ...
        if len(t) <= 1:
            print(f"⚠️ Skipping {battery_files[0].name} — not enough time data (len={len(t)})")
            continue
    ...
    ax[0].set_ylim(
        v_concat.min() - 0.1 * np.ptp(v_concat), 
        v_concat.max() + 0.1 * np.ptp(v_concat)
    )
    ax0_sec.set_ylim(
        c_concat.min() - 0.1 * np.ptp(c_concat), 
        c_concat.max() + 0.1 * np.ptp(c_concat)
    )
    ax[1].set_ylim(
    ...
    ax[0].set_title(f"Voltage/Current Profile Group: {time_id} - Mode: {mode}")
    ...
    ax[0].legend(loc="upper left", bbox_to_anchor=(1.10, 1.0), title="Voltage")
    ax0_sec.legend(loc="upper left", bbox_to_anchor=(1.10, 0.55), title="Current")
    ax[1].legend(loc="upper left", bbox_to_anchor=(1.10, 0.75), title="SOC")
