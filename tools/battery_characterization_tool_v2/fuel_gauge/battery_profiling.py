...
def continuous_rational_split_two_breaks(x, a1, b1, c1, d1, a2, b2, c2, d2, a3, b3, c3):
    """
...
def time_to_minutes(time, offset=None):
    # print(f"type(discharge_profile.time): {type(time)}")
    if np.isscalar(time):
        time = np.array([time])
    if offset is None:
...
def correlator(data_vector):

    fir_half_len = 20
    corr = np.concat(
        (np.zeros(fir_half_len, dtype=int) - 1, np.zeros(fir_half_len, dtype=int) + 1),
        axis=None,
    )
    return signal.convolve(data_vector, corr, mode="same", method="direct")
...
def estimate_R_int(time, ibat, vbat, temp, debug=False):
    """
    Estimate the internal resistance of the battery
    """

    # Identify measuring marks
    fir_len = 40
    offset_left = 10
    offset_right = 45
    ...
    # Use convolution with correlation filter of "step" shape to
    # pull out the signal load transitions.
    ibat_corr = correlator(ibat)
    peaks_indeces, _ = signal.find_peaks(abs(ibat_corr), prominence=300)
    peaks_cut = peaks_indeces[
        np.logical_and(
            peaks_indeces - offset_left > 0, peaks_indeces + offset_right < len(ibat)
        )
    ]
    transitions_idx_np = np.array(peaks_cut)

    # Look left and right by given offset from identified transitions
    m1_idx = transitions_idx_np - offset_left  # Marks 1
    m2_idx = transitions_idx_np + offset_right  # Marks 2

    # Sample the waveform at marks and estimate the internal resistance
    R_est = []  # estimation for raw V_t
    R_est_filtered = []  # estimation for filtered V_t

    for i in range(0, len(transitions_idx_np)):

        # estim = (vbat[m2_idx[i]] - vbat[m1_idx[i]]) / (ibat[m1_idx[i]]/1000 - ibat[m2_idx[i]]/1000)
        estim = (vbat[m2_idx[i]] - vbat[m1_idx[i]]) / (
            ibat[m1_idx[i]] / 1000 - ibat[m2_idx[i]] / 1000
        )

        # TODO: Maybe I should shift the marks for filtered waveform to compensate for FIR filter propagation delay??
        estim_filtered = (vbat_filtered[m2_idx[i]] - vbat_filtered[m1_idx[i]]) / (
            (ibat_filtered[m1_idx[i]]) / 1000 - ibat_filtered[m2_idx[i]] / 1000
        )

        R_est.append(estim)
        R_est_filtered.append(estim_filtered)
    ...
    if len(R_est_filtered_np) < 3:
        ...
        R_est_final_filtered = sum(R_est_filtered_np[1:-1]) / len(
            R_est_filtered_np[1:-1]
        )
    ...
    if debug:
        ...
        fig.suptitle(
            "Rin identification on switching load profile - " + str(temp) + "C",
            fontsize=8,
        )

        ax[0].plot(time_to_minutes(time), vbat, label="V_bat", linewidth=0.7)
        ax[0].plot(
            time_to_minutes(time), vbat_filtered, label="V_bat_filtered", linewidth=1
        )
        ...
        ax[1].plot(
            time_to_minutes(time), ibat_filtered, label="I_bat_filtered", linewidth=1
        )
        ax[1].plot(
            time_to_minutes(time[transitions_idx_np], time[0]),
            ibat_filtered[transitions_idx_np],
            "x",
            markersize=3,
            color="red",
        ...
        ax[1].plot(
            time_to_minutes(time[m1_idx], time[0]),
            ibat_filtered[m1_idx],
            "x",
            markersize=3,
            color="magenta",
            label="V_t1",
        ...
        ax[1].plot(
            time_to_minutes(time[m2_idx], time[0]),
            ibat_filtered[m2_idx],
            "x",
            markersize=3,
            color="lime",
            label="V_t2",
        ...
        ax[2].plot(
            time_to_minutes(time[peaks_indeces], time[0]),
            ibat_corr[peaks_indeces],
            "x",
            markersize=3,
            color="red",
            label="Load transitions",
        ...
        ax[0].plot(
            time_to_minutes(time), ibat_filtered, linewidth=0.7, label="I_bat_filtered"
        )
        ...
        ax[1].axhline(
            ...
            color="r",
            linestyle="--",
            label=f"R_int averaged {R_est_final:.2f} ohms",
        )
        ax[1].axhline(
            y=R_est_final_filtered,
            color="g",
            linestyle="--",
            label=f"R_int_filtered averaged: {R_est_final_filtered:.2f} ohms",
        )
        ax[1].legend()
...
def fit_R_int_curve(r_int_arr, temp_arr):
    ...
    popt, pcov = curve_fit(poly_2ord, temp_arr, r_int_arr, p0=p0, method="trf")

    p0 = [1.0] * 4

    poptr, pcovr = curve_fit(rational_fit, temp_arr, r_int_arr, p0=p0, method="trf")
...
def fit_soc_curve(soc_curve, graph_title="SoC Curve Fit"):
    """
    Fit a SoC vs OCV curve using a rational-linear-rational model and plot it.

    Parameters:
    - soc_curve: 2D array [SoC, OCV]
    - graph_title: Title of the generated plot (string)
    ...
    popt_rlr, pcov_rlr = curve_fit(
        rational_linear_rational, soc_curve[0], soc_curve[1], p0=p0, method="trf"
    )
...
def extract_SoC_interpolation_points(
    V_OC, I_OC, T_OC, measured_capacity, num_of_intervals, profile_ascending=False
):
    """
    ...
    for i in range(len(intervals)):
        ...
        for j in range(len(I_OC)):
            ...
            cur_incr = ((I_OC[rev_i - 1] + I_OC[rev_i]) / 2) * (
                (T_OC[rev_i] - T_OC[rev_i - 1]) / 1000
            )
...
def extract_SoC_curve(
    ...
    SoC_curve, SoC_curve_indeces = extract_SoC_interpolation_points(
        V_oc_cut, I_oc_cut, cut_time, effective_capacity, num_of_points
    )
    ...
    if debug:
        ...
        fig.suptitle("Linear discharge profile - " + str(test_name) + "C")

        ax[0].set_title("Open-circuit voltage", fontsize=6)
        ax[0].plot(
            time_to_minutes(linear_dischg_data.time), V_oc, linewidth=0.7, label="V_oc"
        )
        ax[0].plot(
            time_to_minutes(linear_dischg_data.time),
            linear_dischg_data.vbat,
            linewidth=0.7,
            label="V_bat",
        )
        ax[0].plot(
            time_to_minutes(cut_time),
            V_oc_cut,
            linewidth=0.7,
            label="V_oc effective area",
            color="red",
        )
        ax[0].set_xlabel("Time [min]", fontsize=6)
        ax[0].set_ylabel("Voltage [V]", fontsize=6)
        ax[0].set_xlim(
            [
                0,
                time_to_minutes(
                    linear_dischg_data.time[-1], linear_dischg_data.time[0]
                ),
            ]
        )
        ...
        for ip in SoC_curve_indeces:
            ax[0].axvline(
                x=time_to_minutes(cut_time[ip], cut_time[0]),
                color="gray",
                linestyle="--",
                linewidth=0.5,
                alpha=0.3,
        ...
        ax[1].plot(
            time_to_minutes(cut_time),
            I_oc_cut,
            label="I_bat_filtered effective area",
            linewidth=0.7,
            color="red",
        )
        ax[1].set_xlabel("Time [min]", fontsize=6)
...
def extract_SoC_curve_charging(
    test_name,
    linear_dischg_data,
    Rint,
    max_chg_voltage,
    ...
    total_capacity_charge = coulomb_counter(
        linear_dischg_data.time, linear_dischg_data.ibat
    )
    effective_capacity = coulomb_counter(cut_time, I_oc_cut)
    # effective_capacity = total_capacity
    ...
    if debug:
        ...
        fig.suptitle("Linear charge profile - " + str(test_name) + "C")
    ...
    return SoC_curve, total_capacity_charge, effective_capacity
