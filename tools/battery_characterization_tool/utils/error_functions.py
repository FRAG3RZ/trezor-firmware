...
def compute_soc_error(
    ...
    for i in range(start_idx, start_idx + smoothed_length):
        ...
        point_weight = 1.0  # if variation <= current_tolerance_mA else 0

        # Override penalty near the edges (bottom 15% and top 15%)
        lower_thresh = 0.15 * smoothed_length
        upper_thresh = 0.85 * smoothed_length
        if smoothed_idx < lower_thresh or smoothed_idx > upper_thresh:
            point_weight = 4.0
...
def summarize_simulation_errors(
    ...
    for est in estimators_to_plot:
        slope_col = f"avg_avg_{est}_slope_error"
        soc_col = f"avg_{est}_end_soc_error"
        cum_slope_col = f"cumulative_{est}_slope_error"
        cum_soc_col = f"cumulative_{est}_end_soc_error"
        cum_dev_col = f"cumulative_{est}_deviation_error"

        if slope_col in summary_df.columns and soc_col in summary_df.columns and cum_dev_col in summary_df.columns:
            slope_max = summary_df[slope_col].max()
            soc_max = summary_df[soc_col].max()
            cum_slope_max = summary_df[cum_slope_col].max()
            cum_soc_max = summary_df[cum_soc_col].max()
            cum_dev_max = summary_df[cum_dev_col].max()

            if not any(np.isnan([slope_max, soc_max, cum_slope_max, cum_soc_max, cum_dev_max])):
                # normalize non-cumulative
                slope_norm = summary_df[slope_col] / slope_max if slope_max != 0 else 0
                soc_norm   = summary_df[soc_col]   / soc_max   if soc_max   != 0 else 0

                # normalize cumulative
                cum_slope_norm = summary_df[cum_slope_col] / cum_slope_max if cum_slope_max != 0 else 0
                cum_soc_norm   = summary_df[cum_soc_col]   / cum_soc_max   if cum_soc_max   != 0 else 0
                cum_dev_norm   = summary_df[cum_dev_col]   / cum_dev_max   if cum_dev_max   != 0 else 0

                # weighted averages
                summary_df[f"Combined error weights - {est}"] = (2 * soc_norm + slope_norm) / 3
                summary_df[f"Combined cumulative error weights - {est}"] = (2 * cum_soc_norm + cum_slope_norm) / 3

                # combined metric with cumulative deviation included
                summary_df[f"Combined cumulative & deviation error weights - {est}"] = (
                    summary_df[f"Combined cumulative error weights - {est}"] + 2*cum_dev_norm
                ) / 3   
    ...
    correlation = summary_df[["Combined error weights - ekf1", "Combined cumulative error weights - ekf1"]].corr()
    print(f"\nCorrelation between combined error weights and cumulative combined error weights:\n{correlation}", flush=True)

    correlation = summary_df[["Combined error weights - ekf1", "Combined cumulative & deviation error weights - ekf1"]].corr()
    print(f"\nCorrelation between combined error weights and cumulative & deviation error weights:\n{correlation}", flush=True)
    ...
    for est in estimators_to_plot:
        ...
        scatter_metrics = [
            ...
            f"Combined cumulative & deviation error weights - {est}"
        ]
